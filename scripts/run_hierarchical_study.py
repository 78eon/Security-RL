#!/usr/bin/env python3
"""Freeze, validate and run the Phase 12 matched hierarchical-policy study."""
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/hierarchical_policy.yaml"),
    )
...
def project_parallel_training_minutes(
    ...
    *,
    ...
    if workers <= 0 or seeds <= 0 or set(elapsed_by_arm) != set(arms):
        raise HierarchicalStudyError("parallel runtime projection inputs are incomplete")
    ...
    if any(not math.isfinite(value) or value <= 0 for value in durations.values()):
        raise HierarchicalStudyError("parallel runtime timings must be finite and positive")
    slots = [0.0] * workers
    heapq.heapify(slots)
...
def validate_development_gate(path: Path, config: HierarchicalResearchConfig) -> dict:
    if not path.is_file():
    ...
    except (KeyError, TypeError, ValueError, HierarchicalStudyError) as exc:
    ...
    if failures:
        raise HierarchicalStudyError(
            "Phase 12 development gate failed:\n  " + "\n  ".join(failures)
        )
    metadata["parallel_projected_canonical_minutes"] = projected
    return metadata


def run_study(args: argparse.Namespace, config: HierarchicalResearchConfig) -> dict:
    development = args.action == "development"
    if args.timesteps is not None and not development:
        raise HierarchicalStudyError("--timesteps is allowed only for development")
    if args.limit_topologies and not development:
        raise HierarchicalStudyError("canonical test topology set cannot be limited")
    ...
    if not report["complete"]:
        raise HierarchicalStudyError("Phase 12 study is missing a matched arm/seed")
    elapsed_by_arm = {
        arm: sum(
    ...
    (result_root / "README.md").write_text(
        "# Phase 12 hierarchical-policy study\n\n"
...
def main() -> None:
    ...
    config = HierarchicalResearchConfig.from_yaml(args.config)
