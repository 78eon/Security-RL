#!/usr/bin/env python3
"""Freeze, validate and run the Phase 11 matched transfer study."""
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/transfer_learning.yaml"),
    )
    ...
    result.add_argument("--source-timesteps", type=int, help="development action only")
    result.add_argument("--target-timesteps", type=int, help="development action only")
...
def project_parallel_training_minutes(
    ...
    *,
    ...
    if workers <= 0 or seeds <= 0 or set(elapsed_by_arm) != set(arms):
        raise TransferStudyError("parallel runtime projection inputs are incomplete")
    ...
    if any(not math.isfinite(value) or value <= 0 for value in durations.values()):
        raise TransferStudyError("parallel runtime timings must be finite and positive")
    slots = [0.0] * workers
    heapq.heapify(slots)
...
def _train_job(payload: tuple) -> dict:
    run_dir, arm, seed, config, frozen, source_timesteps, target_timesteps = payload
    return train_arm(
        run_dir,
        arm=arm,
        training_seed=seed,
...
def _manifest_elapsed(manifest: dict) -> float:
    source = manifest.get("source_pretraining") or {}
    return float(manifest["target_training_elapsed_seconds"]) + float(
        source.get("training_elapsed_seconds", 0.0)
    )


def validate_development_gate(path: Path, config: TransferResearchConfig) -> dict:
    if not path.is_file():
        raise TransferStudyError("canonical run requires the full excluded-seed development gate")
    metadata = json.loads(path.read_text())
    checks = {
        "phase": (metadata.get("phase"), "development"),
        "complete": (metadata.get("complete"), True),
        "scientific config": (metadata.get("scientific_config_hash"), config.scientific_digest()),
        "source budget": (
            metadata.get("source_pretrain_timesteps"),
            config.source_pretrain_timesteps,
        ),
        "target budget": (
            metadata.get("target_adaptation_timesteps"),
            config.target_adaptation_timesteps,
        ),
    ...
    except (KeyError, TypeError, ValueError, TransferStudyError) as exc:
    ...
    if failures:
        raise TransferStudyError("Phase 11 development gate failed:\n  " + "\n  ".join(failures))
    metadata["parallel_projected_canonical_minutes"] = projected
    return metadata


def run_study(args: argparse.Namespace, config: TransferResearchConfig) -> dict:
    development = args.action == "development"
    if (args.source_timesteps is not None or args.target_timesteps is not None) and not development:
        raise TransferStudyError("budget overrides are allowed only for development")
    if args.limit_topologies and not development:
        raise TransferStudyError("canonical test topology set cannot be limited")
    ...
    source_timesteps = args.source_timesteps or config.source_pretrain_timesteps
    target_timesteps = args.target_timesteps or config.target_adaptation_timesteps
    ...
    if not report["complete"]:
        raise TransferStudyError("Phase 11 study is missing a matched arm/seed")
    elapsed_by_arm = {
        arm: sum(_manifest_elapsed(item) for item in training_manifests if item["arm"] == arm)
        for arm in config.arms
    }
    ...
    metadata = {
        ...
        "source_pretrain_timesteps": source_timesteps,
        "target_adaptation_timesteps": target_timesteps,
    ...
    (result_root / "README.md").write_text(
        "# Phase 11 transfer-learning study\n\n"
    ...
    if (
        development
        and source_timesteps == config.source_pretrain_timesteps
        and target_timesteps == config.target_adaptation_timesteps
...
def main() -> None:
    ...
    config = TransferResearchConfig.from_yaml(args.config)
