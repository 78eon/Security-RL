#!/usr/bin/env python3
"""Freeze, validate and run the Phase 9 matched curriculum study."""
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--config",
        type=Path,
        default=Path("configs/experiments/curriculum_learning.yaml"),
    )
...
def project_parallel_training_minutes(
    ...
    *,
    ...
    if workers <= 0 or seeds <= 0 or set(elapsed_by_arm) != set(arms):
        raise CurriculumStudyError("parallel runtime projection inputs are incomplete")
    ...
    if any(not math.isfinite(value) or value <= 0 for value in durations.values()):
        raise CurriculumStudyError("parallel runtime timings must be finite and positive")
    slots = [0.0] * workers
    heapq.heapify(slots)
...
def validate_development_gate(path: Path, config: CurriculumResearchConfig) -> dict:
    if not path.is_file():
        raise CurriculumStudyError("canonical run requires the full excluded-seed development gate")
    ...
    except (KeyError, TypeError, ValueError, CurriculumStudyError) as exc:
    ...
    if failures:
        raise CurriculumStudyError("Phase 9 development gate failed:\n  " + "\n  ".join(failures))
    metadata["parallel_projected_canonical_minutes"] = projected
    return metadata


def run_study(args: argparse.Namespace, config: CurriculumResearchConfig) -> dict:
    development = args.action == "development"
    if args.timesteps is not None and not development:
        raise CurriculumStudyError("--timesteps is allowed only for development")
    if args.limit_topologies and not development:
        raise CurriculumStudyError("canonical test topology set cannot be limited")
    ...
    if not report["complete"]:
        raise CurriculumStudyError("Phase 9 study is missing a matched arm/seed")
    elapsed_by_arm = {
        arm: sum(
    ...
    (result_root / "README.md").write_text(
        "# Phase 9 curriculum-learning study\n\n"
...
def main() -> None:
    ...
    config = CurriculumResearchConfig.from_yaml(args.config)
